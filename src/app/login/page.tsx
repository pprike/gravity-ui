...
export default function LoginPage() {
  ...
  const [tenantSlug, setTenantSlug] = useState("tenant-a");
  ...
  async function handleSubmit(event: React.FormEvent) {
    ...
    try {
      const path = await login({ tenantSlug, email, password });
      router.replace(path);
    } catch (err) {
      const message =
    ...
    <div className="flex min-h-screen items-center justify-center bg-gradient-to-br from-neutral-50 via-white to-primary-50 px-4 py-12">
      <div className="w-full max-w-md space-y-6">
        <div className="text-center">
          ...
          <h1 className="mt-2 text-display text-neutral-900">Sign in</h1>
        ...
        <p className="text-center text-caption text-neutral-500">
          Invited to join?{" "}
          <Link href="/activate" className="font-medium text-primary-600 hover:text-primary-700">
