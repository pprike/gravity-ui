  ...
  if (error || !data) {
    return (
      <div className="flex flex-col items-center justify-center rounded-xl border border-dashed border-neutral-300 bg-neutral-50 px-6 py-16 text-center">
        ...
        <h2 className="text-lg font-semibold text-neutral-900">Dashboard unavailable</h2>
  ...
  if (isEmpty) {
    return (
      <div className="space-y-6">
        <DashboardHeader firstName={firstName} locationName={locationName} />
        <div className="flex flex-col items-center justify-center rounded-xl border border-dashed border-neutral-300 bg-neutral-50 px-6 py-16 text-center">
          <div className="mb-4 flex h-14 w-14 items-center justify-center rounded-full bg-primary-50 text-primary-600">
            <Calendar className="h-7 w-7" aria-hidden />
          </div>
          <h2 className="text-lg font-semibold text-neutral-900">Your dashboard is ready</h2>
          <p className="mt-2 max-w-md text-sm text-neutral-600">
            Create membership plans and schedule classes to start seeing business
            metrics here.
          </p>
          <Button className="mt-6" type="button" onClick={() => router.push("/schedule/new")}>
            Create Class
          </Button>
        </div>
      </div>
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-5 sm:grid-cols-2 xl:grid-cols-4">
        <KpiCard
          ...
          positive={data.activeMemberships.count >= data.activeMemberships.priorCount}
        />
        <KpiCard
      ...
      <div className="grid gap-6 xl:grid-cols-[minmax(0,1.6fr)_minmax(0,1fr)]">
        ...
        <Card className="p-0">
          <PanelHeader title="Classes Scheduled Today" href="/schedule" />
          <ul className="divide-y divide-neutral-100">
    ...
    <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
      <div>
        ...
        <p className="mt-1 text-sm text-slate-500">
          {locationName
            ? `Executive overview for ${locationName}.`
            : "Executive overview of your studio today."}
        </p>
      </div>
